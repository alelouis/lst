...
pub fn print_lst(lst: f64) {
    // Print an LST in hours:minutes:seconds format
    let (gmst_h, gmst_m, gmst_s) = deg_to_hms(lst);
    println!("LST:\t{:0>2.0}:{:0>2.0}:{:0>2.0}", gmst_h, gmst_m, gmst_s);
}
